...
某些操作需要用户登录后才能进行，这个不用多解释了。

传统的COOKIE+SESSION_ID的策略已经过时了，因为**它需要每次在获取用户的SESSION_ID后进行一次查询（无论是查数据库还是REDIS，都需要进行一次查询，以获取用户当前的身份和权限**。而更好的做法是JWT，它的本质是一个服务端在登录后构建一个JSON+签名（这里可以用对称或者不对称，用不对称更好，即服务端的私钥去签名），返回给前端，之后前端拿到这个JSON的BASE64字串，就可以存入到请求头内，服务端接收后，每次都要用公钥去解密签名，验证有效性，之后再解密JSON，拿到用户的信息和角色，**就可以跳过数据库查询，直接基于角色进行业务活动了**。
...
每一个部分都是BASE64编码，所以整个部分就是一个：`FOO.BAR.BARTZ`的过程。服务端拿到后，处理如下：

1. 分离出header，payload和签名
2. 对签名进行公钥解密，对比header和payload的组合，验证是否有篡改
3. 如果没有篡改，对payload反BASE64，拿到真实JSON，确认当前用户身份，角色，是否过期等等
4. 如果一切顺利，执行后续业务操作，如果不顺利，操作被拦截

由于需要处理过期问题，前端需要在AXIOS里面添加一个拦截器，即当用户的身份过期（一般是5~30分钟）后，再发一个请求去获取新的 JWT，并存入到本地缓存，这个拦截器一般需要前端去实现。

所以前端的拦截器需要这样处理：

- 发送一个请求校验当前JWT是否有效
- 如果有效，把JWT添加到请求头内，并继续流程

- 如果无效，发送一个刷新JWT的请求，得到新的JWT后，把新的JWT替换掉当前的缓存的JWT，之后添加到请求头内，并继续流程
- 如果无效，且无法刷新JWT，则表示用户进入异常流程，应该回到登录页面（或者让服务端返回REDIRECT响应，来控制客户端的跳转）

NEXTJS使用next-auth.js库来处理登录和权限校验的问题，对于13及以上版本，应该使用NEXT-AUTH的V5版本，之前的应该采用V4版本。V5目前还是BETA版但是可以用。

感觉NEXTJS官方的教程不够看，特别是AUTH的部分写得太模糊了，比REACT得教程差太多，所以还是决定去看油管视频学习。
